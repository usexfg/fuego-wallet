...
class _PeerSwapScreenState extends State<PeerSwapScreen> {
      ...
      builder: (context, state) {
        ...
        return SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            if (state.lastResult != null) ...[const SizedBox(height: 12), _statusCard(state.lastResult!, false)],
            if (state.error != null) ...[const SizedBox(height: 12), _statusCard(state.error!, true)],
            if (active.isNotEmpty) ...[const SizedBox(height: 20), _activeSection(active)],
            if (history.isNotEmpty) ...[const SizedBox(height: 20), _historySection(history)],
          ]),
        );
      },
    );
  ...
  Widget _whatIsThis() => Container(
        ...
        child: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Icon(Icons.handshake_outlined, color: AppTheme.primaryColor, size: 18),
            SizedBox(width: 8),
            Text('Direct peer swap', style: TextStyle(color: AppTheme.primaryColor, fontSize: 13, fontWeight: FontWeight.w700)),
          ]),
          SizedBox(height: 8),
          Text(
  ...
  Widget _form(DexState state, String ticker, List active) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      const SizedBox(height: 16),
      ElevatedButton(
        onPressed: (state.isSwapInitiating || !connected) ? null : () => _initiateSwap(state, ticker),
        ...
        child: state.isSwapInitiating
            ...
            : const Text('INITIATE SWAP', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
      ),
    ]);
  }
  ...
  Widget _statusCard(String text, bool isError) => Container(
        ...
        decoration: BoxDecoration(
          color: (isError ? AppTheme.errorColor : AppTheme.successColor).withValues(alpha: 0.1),
          borderRadius: BorderRadius.circular(8),
        ),
        child: Text(text, style: TextStyle(color: isError ? AppTheme.errorColor : AppTheme.successColor, fontSize: 12)),
      );

  Widget _activeSection(List active) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Active Swaps', style: TextStyle(color: AppTheme.textPrimary, fontSize: 14, fontWeight: FontWeight.w600)),
        const SizedBox(height: 8),
        ...active.map((swap) => Container(
  ...
  Widget _historySection(List history) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ...history.map((swap) {
          ...
          return Container(
            ...
            decoration: BoxDecoration(color: AppTheme.cardColor, borderRadius: BorderRadius.circular(8)),
            child: Row(children: [
              Icon(isRefunded ? Icons.replay : Icons.check_circle,
                  color: isRefunded ? AppTheme.warningColor : AppTheme.successColor, size: 16),
              const SizedBox(width: 8),
              Expanded(child: Text('${swap.xfgAmountDecimal.toStringAsFixed(2)} XFG \u2192 ${swap.pairName}',
